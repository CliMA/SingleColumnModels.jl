...
output_dir = joinpath(scm_dir, "output", "TRMM_LBA")
mkpath(output_dir)
...
data_file = Dataset(joinpath(PyCLES_output_dataset_path, "TRMM_LBA.nc"), "r")

@testset "TRMM_LBA EDMF Solution Quality Assurance (QA) tests" begin

    grid, q, aux, params =
        scm.EDMF.run(param_set, scm.EDMF.TRMM_LBA(), output_dir)
