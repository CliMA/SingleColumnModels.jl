...
@testset "DomainSubSet, two domains" begin
  domain_set,domain_subset = DomainSet(gm=1,en=1),DomainSubSet(gm=true,en=true)
  idx = DomainIdx(domain_set)
  idx_ss = DomainIdx(domain_set, domain_subset)
  a_map = DD.get_sv_a_map(idx, idx_ss)
  gm, en, ud, sd, al = allcombinations(idx)
  ...
  domain_set,domain_subset = DomainSet(gm=1,ud=3),DomainSubSet(gm=true,ud=true)
  idx = DomainIdx(domain_set)
  idx_ss = DomainIdx(domain_set, domain_subset)
  a_map = DD.get_sv_a_map(idx, idx_ss)
  gm, en, ud, sd, al = allcombinations(idx)
  ...
  domain_set,domain_subset = DomainSet(en=1,ud=3),DomainSubSet(en=true,ud=true)
  idx = DomainIdx(domain_set)
  idx_ss = DomainIdx(domain_set, domain_subset)
  a_map = DD.get_sv_a_map(idx, idx_ss)
  gm, en, ud, sd, al = allcombinations(idx)
  ...
  domain_set,domain_subset = DomainSet(gm=1,ud=1),DomainSubSet(gm=true,ud=true)
  idx = DomainIdx(domain_set)
  idx_ss = DomainIdx(domain_set, domain_subset)
  a_map = DD.get_sv_a_map(idx, idx_ss)
  gm, en, ud, sd, al = allcombinations(idx)
  ...
  domain_set,domain_subset = DomainSet(en=1,ud=1),DomainSubSet(en=true,ud=true)
  idx = DomainIdx(domain_set)
  idx_ss = DomainIdx(domain_set, domain_subset)
  a_map = DD.get_sv_a_map(idx, idx_ss)
  gm, en, ud, sd, al = allcombinations(idx)
...
@testset "DomainIdx, all domains" begin
  domain_set, domain_subset = DomainSet(gm=1,en=1,ud=4), DomainSubSet(gm=true,en=true,ud=true)
  idx = DomainIdx(domain_set)
  idx_ss = DomainIdx(domain_set, domain_subset)
  a_map = DD.get_sv_a_map(idx, idx_ss)
  gm, en, ud, sd, al = allcombinations(idx)
...
@testset "Test DomainSubSet indexing, multiple domains" begin
  domain_set, domain_subset = DomainSet(gm=1,en=1,ud=4), DomainSubSet(gm=true)
...
@testset "Test global indexing, many updrafts" begin
  ...
  idx_ss_per_var = Dict([name => DomainIdx(domain_set, dss_per_var[name]) for name in var_names]...)
  a_map = Dict([name => DD.get_sv_a_map(idx, idx_ss_per_var[name]) for name in var_names]...)
  gm, en, ud, sd, al = allcombinations(idx)
  ...
  @test_throws BoundsError DD.var_suffix(vm, idx, idx_ss_per_var[:a], :a, 1000)
