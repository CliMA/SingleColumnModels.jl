...
function DomainDecomposition(vars, domain_set::DomainSet) where {T}
  ...
  idx_ss_per_var = Dict(ϕ => DomainIdx(domain_set, dss_per_var[ϕ]) for ϕ in var_names)
  a_map = Dict(ϕ => get_sv_a_map(idx, idx_ss_per_var[ϕ]) for ϕ in var_names)

  return DomainDecomposition{typeof(var_mapper)}(idx,
                                                 var_mapper,
                                                 idx_ss_per_var,
                                                 a_map,
                                                 sd_unmapped,
...
allcombinations(domain_decomp::DomainDecomposition) = allcombinations(DomainIdx(domain_decomp))
