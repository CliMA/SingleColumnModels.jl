...
function Params(param_set, ::BOMEX)
  ...
  params[:ρg] = air_density(param_set, params[:Tg], params[:Pg], PhasePartition(params[:qtg]))
  params[:αg] = 1/params[:ρg]
  params[:Tsurface] = 299.1 * exner_given_pressure(param_set, params[:Pg])
  params[:qsurface] = 22.45e-3
  params[:lhf] = 5.2e-5 * params[:ρg] * latent_heat_vapor(param_set, params[:Tsurface])
  params[:shf] = 8.0e-3 * cp_m(param_set, PhasePartition(params[:qsurface])) * params[:ρg]
  params[:ρ_tflux] =  params[:shf] /(cp_m(param_set, PhasePartition(params[:qsurface])))
  params[:ρq_tot_flux] = params[:lhf]/(latent_heat_vapor(param_set, params[:Tsurface]))
  params[:ρθ_liq_flux] = params[:ρ_tflux] / exner_given_pressure(param_set, params[:Pg])
