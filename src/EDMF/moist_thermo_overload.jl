...
"""
air_temperature_from_liquid_ice_pottemp_old(param_set::PS, θ_liq_ice::FT, p::FT, q::PhasePartition{FT}=MoistThermodynamics.q_pt_0(FT)) where {FT<:Real, PS} =
  θ_liq_ice*exner_given_pressure(param_set, p, q) + (FT(LH_v0(param_set))*q.liq + FT(LH_s0(param_set))*q.ice) / cp_m(param_set, q)

air_temperature_from_liquid_ice_pottemp_old(param_set::PS, θ_liq_ice::FT, p::FT) where {FT<:Real, PS} =
  air_temperature_from_liquid_ice_pottemp_old(param_set, θ_liq_ice, p, MoistThermodynamics.q_pt_0(FT))
...
function saturation_adjustment_q_tot_θ_liq_ice_old(param_set::PS,
  ...
  if q_tot <= q_v_sat # If not saturated
    ...
    function eos(T)
      ...
      θ_dry = dry_pottemp_given_pressure(param_set, T, p, q_pt)
