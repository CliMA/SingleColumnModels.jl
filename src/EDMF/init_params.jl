...
function Params(param_set, ::BOMEX)
  ...
  params[:EddyDiffusivityModel]   = SCAMPyEddyDiffusivity{FT}(;tke_ed_coeff=0.1,
                                                               prandtl_number=1.0)
...
function Params(param_set, ::Soares)
  FT = Float64
  IT = Int
  params = Dict()
  ...
  params[:EddyDiffusivityModel]   = SCAMPyEddyDiffusivity{FT}(;tke_ed_coeff=0.5,
                                                               prandtl_number=1.0)

  params[:PressureModel]          = SCAMPyPressure{FT}(;buoy_coeff=FT(1.0/3.0),
                                                        drag_coeff=FT(0.375),
                                                        plume_spacing=FT(500.0))
  params[:tke_diss_coeff]         = FT(0.01)
