...
function TurbConv(params, case::Case, output_dir::String)
    ...
    FT = eltype(grid)
    q = StateVec(unkowns, FT, n_points, domain_set)
    aux = StateVec(aux_vars, FT, n_points, domain_set)
    tri_diag = Dict{Symbol, StateVec}()
    tri_diag[:tke] = StateVec(tri_diag_vars, FT, n_points, domain_set)
    tri_diag[:q_tot] = StateVec(tri_diag_vars, FT, n_points, domain_set)
    tri_diag[:θ_liq] = StateVec(tri_diag_vars, FT, n_points, domain_set)
    q_new = deepcopy(q)
    tendencies = deepcopy(q)
    aux_O2 = Dict()
    aux_O2[:tke] = StateVec(q_2MO_vars, FT, n_points, domain_set)
