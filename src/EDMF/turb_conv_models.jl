...
function TurbConv(params, case::Case, output_dir::String)
    ...
    aux_vars = (
        ...
        (:f_rad         , DomainSubSet(gm = true)),
        (:q_ice         , DomainSubSet(gm = true)),
    )
    ...
    tri_diag[:tke] = StateVec(tri_diag_vars, grid, domain_set)
    tri_diag[:q_tot] = StateVec(tri_diag_vars, grid, domain_set)
    tri_diag[:θ_liq] = StateVec(tri_diag_vars, grid, domain_set)
    q_new = deepcopy(q)
