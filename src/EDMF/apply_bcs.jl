...
function apply_bcs!(grid::Grid, q::StateVec, aux::StateVec, params, case::BOMEX)
    ...
    cv_θ_liq = surface_variance(
        ρθ_liq_flux * alpha0LL,
        ρθ_liq_flux * alpha0LL,
        ustar,
        zLL,
        obukhov_length,
    ...
    @inbounds for i in ud
        ...
        UpdVar[i].surface_bc.θ_liq =
            (θ_liq_1 + UpdVar[i].surface_scalar_coeff * sqrt(cv_θ_liq))
        UpdVar[i].surface_bc.q_tot =
            (q_tot_1 + UpdVar[i].surface_scalar_coeff * sqrt(cv_q_tot))
